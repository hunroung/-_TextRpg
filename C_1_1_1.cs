...
namespace WindowsFormsApp1
{
    public partial class C_1_1_1 : Form
    {
        ...
        {
            move_btn_enable();
            textBox1.Text += "당신은 더 깊은 바다를 항해하기 위해 배를 탔다.\r\n";
            this.Refresh();
            Thread.Sleep(2000);
            textBox1.Text += "그렇게 좋던 햇살 아래 갑자기 그림자가 드리운다.\r\n";
            this.Refresh();
            Thread.Sleep(2000);
            textBox1.Text += "위를 보니 엄청난 고래가 물 위로 점프하여 해를 가리고 있다!\r\n";
            this.Refresh();
            Thread.Sleep(2000);
        ...
        private void btn_attack_Click(object sender, EventArgs e)
        {
            ...
            if (k == 1)
            {
                if (whale.defense(ch.attack()) == 1)//변경 필요
                {
                    picture_main.Image = ch.main_attack;
                    picture_npc.Image = whale.img_defend;//변경 필요
                ...
                {
                    ch.skill_re();
                    ch.exp_gain(whale.exp);//변경 필요
                    textBox1.Text += npc_name.Text + "이 죽었다. exp : " + whale.exp.ToString() + " 획득\r\n";
                    picture_npc.Image = whale.img_dead;
                    picture_main.Image = ch.main_attack;

                }
            }

            else if (whale.damaged(ch.attack()) == 1)//변경 필요
            {
                picture_main.Image = ch.main_attack;
                picture_npc.Image = whale.img_attacked;//변경 필요
                update(); //
                ...
                switch (k)
                {
                    case 0://슬라임도 공격을 선택한 경우
                        ...
                        if (ch.damaged(whale.attack()) != 1)// 슬라임 공격으로 사망 //변경 필요
                        {
                            picture_main.Image = ch.main_attacked;
                            picture_npc.Image = whale.img_attack;//변경 필요
                            textBox1.Text += "당신은 죽었다\r\n";
                            picture_npc.Image = whale.img;//변경 필요
                    ...
                    case 2: //슬라임이 스킬을 사용한 경우
                        ...
                        else if (ch.damaged(temp) != 1)//변경 필요
                        {
                            ...
                            picture_npc.Image = whale.img_skill;//변경 필요
                            update();
                            this.Refresh();
            ...
            else
            {
                ...
                ch.exp_gain(whale.exp);//변경 필요
                textBox1.Text += npc_name.Text + "이 죽었다. exp : " + whale.exp.ToString() + " 획득\r\n";//변경 필요
                picture_npc.Image = whale.img_dead;//변경 필요
                picture_main.Image = ch.main_attack;
        ...
        private void btn_skill_Click(object sender, EventArgs e)
        {
            ...
            else if (cmb_skill.SelectedIndex >= 0 && cmb_skill.Items[cmb_skill.SelectedIndex].ToString() != "")
            {
                ...
                if (k == 1)
                {
                    if (whale.defense(ch.skill_use(cmb_skill.Items[cmb_skill.SelectedIndex].ToString())) == 1)//변경 필요
                    {
                ...
                else if (whale.damaged(ch.skill_use(cmb_skill.Items[cmb_skill.SelectedIndex].ToString())) == 1)//변경 필요
                {
                    picture_main.Image = ch.main_skill;
                    picture_npc.Image = whale.img_attacked;//변경 필요
                    update();
        ...
        private void btn_defend_Click(object sender, EventArgs e)
        {
            ...
            switch (k)
            {
                case 0://슬라임이 공격을 선택한 경우
                    ...
                    if (ch.defense(whale.attack()) != 1)// 슬라임 공격으로 사망 //변경 필요
                    {
                        picture_main.Image = ch.main_dead;
                        picture_npc.Image = whale.img_attack;//변경 필요
                        textBox1.Text += "당신은 죽었다\r\n";
